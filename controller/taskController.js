...
const isOwnerExist = (task, userId) => {
  ...
  if (task.owners.length) {
    task.owners.forEach((el) => {
      ...
      if (el._id.toString() === userId.toString()) isOwner = true;
    });
    // then return isOwner
    return isOwner;
  }
...
exports.getTasks = async (req, res, next) => {
  try {
    ...
    }).populate("owners", "fullname image_path");

    if (!tasks.length)
      return res
        .status(404)
        .json({ success: false, error: "There Is No Tasks Found" });
...
exports.updateTask = async function (req, res, next) {
  try {
    ...
    //find the task
    let task = await await Task.findOne({
      _id: taskId,
      project_id: projectId,
      account_id: req.user.account_id._id,
    });
    if (!task)
      return res
        ...
        .json({
          error:
            "task does not exist in " + req.user.account_id.name + " account",
        });
    ...
    return res
      .status(401)
      .json({ success: false, error: "You Don't Have Enough Permission" });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
...
exports.deleteTask = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ data: null, message: "Task Hase Been Deleted", success: true });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
